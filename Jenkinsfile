...
pipeline {
  ...
  stages {
    ...
    stage('Push SNAPSHOT to Nexus') {
          ...
          steps {
              sh "mvn deploy:deploy-file -e -DgroupId=${groupId} -Dversion=${version} -Dpackaging=${packaging} -Dusername=admin -Durl=${nexusUrl}/repository/${nexusRepoSnapshot}/ -Dfile=${filepath} -DartifactId=${artifactId} -DrepositoryId=${mavenRepoId}"
          }
      }
